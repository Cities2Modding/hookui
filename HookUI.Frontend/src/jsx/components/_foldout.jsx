...
const $FoldOut = React.forwardRef(({ react, style, children, level, icon, label, disableFoldout, onToggle, onItemClick }, ref) => {
    ...
    return <div ref={ref} className={foldoutClassNames} style={{ ...nestingStyle, ...style }}>
        <div className="header_h0B header_8H_ item-mouse-states_Fmi item-focused_FuT" onClick={onHeaderClick} onMouseEnter={onHeaderHover}>
            ...
            <div className="header-content_owU header-content_wUX">{label}</div>
            {tintedIconElement}
        </div>
        {contentElement}
    </div>
